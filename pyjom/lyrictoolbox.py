    ...
    if isBilingual:
        for elem in progressbar.progressbar(test):
            ...
            if flag:  # splited!
                ...
                foreignText = text
                nativeText = translate(foreignText, backend=)
...
def lrcToAnimatedAss(musicPath, lrcPath, assPath, translate=True, translate_method='baidu'):
    ...
    if translate:
        translatedList = getTextListTranslated(textList, translate_method=translate_method) # this is taking long time during test. make it redis lru cached!
    else:
