            ...
            if filter_name == "meta":
                ...
                if required_flag:
                    ...
                    for at_least_number, elements in at_leasts:
                        ...
                        hit_count = sum([int(x in file_info[filter_name]) for x in elements])
                ...
                for detected_object, timespans in DOT:
                    ...
                    for timespan in timespans:
                        ...
                        if stop == "FINAL":
                            stop = metadata["duration"] # do we need to modify the "FINAL" into acturally digits?
                ...
                size_limit, ratio_limit, duration_limit = filter_content.get("size_limit",0.2), filter_content.get("ratio_limit",0.3), filter_content.get("duration_limit", 3)
                avaliable_cuts = []
                for framework in file_info[filter_name]:
                    ...
                    height, width = metadata["resolution"]["height"], metadata["resolution"]["width"]
                    total_area = height*width
                    size = area/total_area
                    if size<size_limit: continue
                    ratio = min(frame_width, frame_height)/max(frame_width, frame_height)
                    if ratio < ratio_limit: continue
                    start, end = framework["start"], framework["end"]
                    if end == "FINAL": end = metadata["duration"]
                    duration = end-start
                    if duration < duration_limit: continue
                    # now append your cuts. are they overlapping?
                    framework2 = {"coords": framework["coords"],"timespan":(start, end)}
