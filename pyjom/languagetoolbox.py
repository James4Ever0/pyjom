...
def filterNonChineseOrEnglishOrJapaneseCharacters(char_list: str):
    output = []
    checkers = {"chinese": lambda c: re.match(r'',c) is not None, "english": lambda c: ((c in " "+string_builtin.punctuation) or (re.match(r'[a-zA-Z0-9]',c) is not None)), "japanese": lambda c:re.match(r'[一-龠ぁ-ゔァ-ヴーａ-ｚＡ-Ｚ０-９々〆〤ヶ]', c) is not None}
