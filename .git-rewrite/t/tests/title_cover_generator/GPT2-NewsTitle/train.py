...
import argparse
import logging
from transformers import GPT2Config # freaking changed.
        ...
        for step, batch in enumerate(iter_bar):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.eval_steps > 0 and global_step % args.eval_steps == 0:
                    ...
                    model.train()
        # 每个epoch进行完，则保存模型
        output_dir2 = os.path.join(args.output_dir, "checkpoint") # that's not right.
        output_dir = os.path.join(args.output_dir, "checkpoint2") # that's not right.
...
def set_args():
    ...
    parser.add_argument('--num_train_epochs', default=5, type=int, help='模型训练的轮数') # we only train freaking 5 times? freaking why?
    ...
    parser.add_argument('--seed', type=int, default=2020, help='随机种子') #  what is this freaking seed?
...
def main():
    ...
    # 判断是否使用预训练好的GPT2模型
    output_dir = os.path.join(args.output_dir, "checkpoint2") # that's not right.
    output_dir2 = os.path.join(args.output_dir, "checkpoint2") # that's not right.
    # saved_path = 
    if args.pretrained_model_path: 
        model = GPT2LMHeadModel.from_pretrained(args.pretrained_model_path)
    elif os.path.exists(output_dir) or os.path.exists(output_dir2):
        try:
            model = GPT2LMHeadModel.from_pretrained(output_dir)
        except:
            model = GPT2LMHeadModel.from_pretrained(output_dir2)
    else:
        # 如果没有指定的预训练模型，则初始化模型
