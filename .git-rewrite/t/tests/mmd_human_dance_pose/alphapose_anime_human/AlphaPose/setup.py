...
def get_install_requires():
    install_requires = [
        'six', 'terminaltables', 'scipy', #you may not install opencv-python and other version of scipy (1.10)
        'matplotlib', 'visdom',
...
def is_installed(package_name):
    ...
    # installed_packages =  [x for x in get_environment(None).iter_installed_distributions()]
    for p in get_environment(None).iter_installed_distributions():
        if package_name.replace("-","_") is p.raw_name.replace("-","_"):
            return True
    return False
