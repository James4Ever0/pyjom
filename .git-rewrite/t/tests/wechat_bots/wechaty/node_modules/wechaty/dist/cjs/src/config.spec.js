...
(0, tstest_1.test)('important variables', async (t) => {
    // t.ok('puppet'   in config, 'should exist `puppet` in Config')
    t.ok('apihost' in config_js_1.config, 'should exist `apihost` in Config');
    t.ok('token' in config_js_1.config, 'should exist `token` in Config');
    ...
    t.ok(config_js_1.config.default.DEFAULT_PROTOCOL, 'should export DEFAULT_PROTOCOL');
    t.ok(config_js_1.config.default.DEFAULT_APIHOST, 'should export DEFAULT_APIHOST');
});
(0, tstest_1.test)('validApiHost()', async (t) => {
    const OK_APIHOSTS = [
        'api.chatie.io',
        'chatie.io:8080',
    ];
    ...
    OK_APIHOSTS.forEach(apihost => {
        t.doesNotThrow(() => {
            config_js_1.config.validApiHost(apihost);
        });
...
//   config.puppetInstance(bak)
// })
(0, tstest_1.test)('systemPuppetName ()', async (t) => {
    const WECHATY_PUPPET_ORIG = process.env['WECHATY_PUPPET'];
    delete process.env['WECHATY_PUPPET'];
    t.equal(config_js_1.config.systemPuppetName(), 'wechaty-puppet-wechat4u', 'should get wechaty-puppet-wechat4u as default puppet name');
    process.env['WECHATY_PUPPET'] = 'wechaty-puppet-mock';
    t.equal(config_js_1.config.systemPuppetName(), 'wechaty-puppet-mock', 'should get puppet name from process.env');
