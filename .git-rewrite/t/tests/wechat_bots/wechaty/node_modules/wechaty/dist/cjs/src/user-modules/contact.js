...
const mod_js_2 = require("../user-mixins/mod.js");
const mod_js_3 = require("../sayable/mod.js");
const stringify_filter_js_1 = require("../helper-functions/stringify-filter.js");
const MixinBase = (0, mod_js_2.wechatifyMixin)((0, mod_js_2.poolifyMixin)(mod_js_1.ContactEventEmitter)());
/**
...
class ContactMixin extends MixinBase {
    ...
    static async find(query) {
        config_js_1.log.silly('Contact', 'find(%s)', JSON.stringify(query, stringify_filter_js_1.stringifyFilter));
        if (typeof query === 'object' && query.id) {
            ...
            if (this.wechaty.puppet.currentUserId === query.id) {
                 ...
                 */
                contact = this.wechaty.ContactSelf.load(query.id);
            }
            else {
                contact = this.wechaty.Contact.load(query.id);
            }
        ...
        if (contactList.length > 1) {
            config_js_1.log.warn('Contact', 'find() got more than 1 result: %d total', contactList.length);
        }
        for (const [idx, contact] of contactList.entries()) {
            ...
            if (valid) {
                config_js_1.log.silly('Contact', 'find() contact<id=%s> is valid, return it', idx, contact.id);
                return contact;
            }
            else {
                config_js_1.log.silly('Contact', 'find() contact<id=%s> is invalid, skip it', idx, contact.id);
            }
        }
        config_js_1.log.warn('Contact', 'find() all of %d contacts are invalid', contactList.length);
        return undefined;
    }
    /**
    ...
    static async findAll(query) {
        config_js_1.log.verbose('Contact', 'findAll(%s)', JSON.stringify(query, stringify_filter_js_1.stringifyFilter) || '');
        try {
            ...
            this.wechaty.emitError(e);
            config_js_1.log.error('Contact', 'this.wechaty.puppet.contactFindAll() rejected: %s', e.message);
            return []; // fail safe
        }
    }
    // TODO
    ...
    constructor(id) {
        super();
        this.id = id;
        config_js_1.log.silly('Contact', `constructor(${id})`);
    }
    /**
     * @ignore
     */
     ...
     */
    async say(sayable) {
        config_js_1.log.verbose('Contact', 'say(%s)', sayable);
        const msgId = await (0, mod_js_3.deliverSayableConversationPuppet)(this.wechaty.puppet)(this.id)(sayable);
        if (msgId) {
    ...
    async alias(newAlias) {
        config_js_1.log.silly('Contact', 'alias(%s)', newAlias === undefined
            ? ''
            : newAlias);
        if (!this.payload) {
        ...
        try {
            ...
            if (newAlias && newAlias !== this.payload.alias) {
                config_js_1.log.warn('Contact', 'alias(%s) sync with server fail: set(%s) is not equal to get(%s)', newAlias, newAlias, this.payload.alias);
            }
        }
        catch (e) {
            this.wechaty.emitError(e);
            config_js_1.log.error('Contact', 'alias(%s) rejected: %s', newAlias, e.message);
        }
    }
    async phone(phoneList) {
        config_js_1.log.silly('Contact', 'phone(%s)', phoneList === undefined ? '' : JSON.stringify(phoneList));
        if (!this.payload) {
            throw new Error('no payload');
        }
        ...
        try {
            ...
            this.wechaty.emitError(e);
            config_js_1.log.error('Contact', 'phone(%s) rejected: %s', JSON.stringify(phoneList), e.message);
        }
    }
    async corporation(remark) {
        config_js_1.log.silly('Contact', 'corporation(%s)', remark);
        if (!this.payload) {
            throw new Error('no payload');
        }
        ...
        try {
            ...
            this.wechaty.emitError(e);
            config_js_1.log.error('Contact', 'corporation(%s) rejected: %s', remark, e.message);
        }
    }
    async description(newDescription) {
        config_js_1.log.silly('Contact', 'description(%s)', newDescription);
        if (!this.payload) {
            throw new Error('no payload');
        }
        ...
        try {
            ...
            this.wechaty.emitError(e);
            config_js_1.log.error('Contact', 'description(%s) rejected: %s', newDescription, e.message);
        }
    }
    title() {
        if (!this.payload) {
     ...
     */
    friend() {
        config_js_1.log.verbose('Contact', 'friend()');
     ...
     */
    async avatar() {
        config_js_1.log.verbose('Contact', 'avatar()');
    ...
    async tags() {
        ...
        try {
            ...
            this.wechaty.emitError(e);
            config_js_1.log.error('Contact', 'tags() exception: %s', e.message);
            return [];
        }
    }
    /**
    ...
    async ready(forceSync = false) {
        config_js_1.log.silly('Contact', 'ready() @ %s with id="%s"', this.wechaty.puppet, this.id);
        if (!forceSync && this.isReady()) { // already ready
            config_js_1.log.silly('Contact', 'ready() isReady() true');
            return;
        }
        try {
            this.payload = await this.wechaty.puppet.contactPayload(this.id);
            ...
            this.wechaty.emitError(e);
            config_js_1.log.verbose('Contact', 'ready() this.wechaty.puppet.contactPayload(%s) exception: %s', this.id, e.message);
            throw e;
        }
    }
    /**
    ...
    async readMark(hasRead) {
        try {
            ...
            this.wechaty.emitError(e);
            config_js_1.log.error('Contact', 'readMark() exception: %s', e.message);
        }
    }
    /**
     * @ignore
     ...
     */
    weixin() {
        config_js_1.log.warn('Contact', 'weixin() is deprecated, use `handle()` instead.');
        console.error(new Error().stack);
        return this.payload?.weixin;
    }
}
class ContactImplBase extends (0, mod_js_2.validationMixin)(ContactMixin)() {
}
class ContactImpl extends (0, mod_js_2.validationMixin)(ContactImplBase)() {
}
exports.ContactImpl = ContactImpl;
