...
class EventStreamManager {
    ...
    start(stream) {
        ...
        this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_HEARTBEAT, connectSuccessHeartbeatPayload);
        /**
    ...
    obj) {
        log.verbose('EventStreamManager', 'grpcEmit(%s[%s], %s)', EventTypeRev[type], type, JSON.stringify(obj));
        ...
        if (this.eventStream) {
            ...
            /**
              ...
              */
            log.warn('EventStreamManager', 'grpcEmit(%s, %s) this.eventStream is undefined.', type, JSON.stringify(obj));
        }
    }
    connectPuppetEventToStreamingCall() {
        ...
        const offAll = () => {
            log.verbose('EventStreamManager', 'connectPuppetEventToStreamingCall() offAll() %s callbacks', offCallbackList.length);
        ...
        for (const eventName of eventNameList) {
            log.verbose('EventStreamManager', 'connectPuppetEventToStreamingCall() this.puppet.on(%s) (listenerCount:%s) registering...', eventName, this.puppet.listenerCount(eventName));
            switch (eventName) {
                case 'dong': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_DONG, payload);
                ...
                case 'dirty': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_DIRTY, payload);
                ...
                case 'error': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_ERROR, payload);
                ...
                case 'heartbeat': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_HEARTBEAT, payload);
                ...
                case 'friendship': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_FRIENDSHIP, payload);
                ...
                case 'login': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_LOGIN, payload);
                ...
                case 'logout': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_LOGOUT, payload);
                ...
                case 'message': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_MESSAGE, payload);
                ...
                case 'post': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_POST, payload);
                ...
                case 'ready': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_READY, payload);
                ...
                case 'room-invite': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_ROOM_INVITE, payload);
                ...
                case 'room-join': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_ROOM_JOIN, payload);
                ...
                case 'room-leave': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_ROOM_LEAVE, payload);
                ...
                case 'room-topic': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_ROOM_TOPIC, payload);
                ...
                case 'scan': {
                    const listener = (payload) => this.grpcEmit(grpcPuppet.EventType.EVENT_TYPE_SCAN, payload);
    ...
    onStreamingCallEnd(removePuppetListeners) {
        ...
        this.eventStream.on('cancelled', () => {
            log.verbose('EventStreamManager', 'this.onStreamingCallEnd() this.eventStream.on(cancelled) fired with arguments: %s', JSON.stringify(arguments));
