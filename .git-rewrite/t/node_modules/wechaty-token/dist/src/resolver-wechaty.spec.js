...
const resolver_wechaty_1 = require("./resolver-wechaty");
tstest_1.test('wechaty resolver smoke testing', async (t) => {
    const TOKEN = '__token__';
    const HOST = '10.1.2.3';
    const PORT = 1024;
    const sandbox = tstest_1.sinon.createSandbox();
    const discoverStub = sandbox.stub(wechaty_token_1.WechatyToken.prototype, 'discover');
    discoverStub.resolves({
        host: HOST,
        port: PORT,
    });
    resolver_wechaty_1.WechatyResolver.setup();
    const target = grpc_js_1.resolverManager.mapUriDefaultScheme(grpc_js_1.parseUri(`wechaty:///${TOKEN}`));
    ...
    const resolver = grpc_js_1.resolverManager.createResolver(target, listener, {});
...
tstest_1.test('wechaty resolver with custom authority', async (t) => {
    ...
    const sandbox = tstest_1.sinon.createSandbox();
    sandbox.stub(wechaty_token_1.WechatyToken.prototype, 'discover')
        .callsFake(function () {
    ...
    resolver_wechaty_1.WechatyResolver.setup();
    const target = grpc_js_1.resolverManager.mapUriDefaultScheme(grpc_js_1.parseUri(`wechaty://${AUTHORITY}/token`));
    const listener = {
        onError: () => { },
