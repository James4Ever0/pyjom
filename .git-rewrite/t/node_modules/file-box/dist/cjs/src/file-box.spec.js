...
exports.TestFileBox = exports.FixtureFileBox = void 0;
...
(0, tstest_1.test)('File smoke testing', async (t) => {
    t.throws(() => file_box_js_1.FileBox.fromFile('x'), 'should throw for a non-existing file');
});
...
class TestFileBox {
    static testFileCreateLocal(localFileFixture) {
        const file = file_box_js_1.FileBox.fromFile(localFileFixture);
        (0, tstest_1.test)('File.createLocal()', async (t) => {
            t.ok(file, 'ok');
        });
        (0, tstest_1.test)('File.fromRemote()', async (t) => {
...
(0, tstest_1.test)('toBase64()', async (t) => {
    const BASE64_DECODED = 'FileBoxBase64\n';
    const BASE64_ENCODED = 'RmlsZUJveEJhc2U2NAo=';
    const fileBox = file_box_js_1.FileBox.fromBase64(BASE64_ENCODED, 'test.txt');
...
(0, tstest_1.test)('fromBuffer() & toBase64()', async (t) => {
    const BASE64_ENCODED = 'RmlsZUJveEJhc2U2NAo=';
    const buffer = Buffer.from(BASE64_ENCODED, 'base64');
    const fileBox = file_box_js_1.FileBox.fromBuffer(buffer, 'test.txt');
    const base64 = await fileBox.toBase64();
    t.equal(base64, BASE64_ENCODED, 'should get base64 back from buffer');
});
(0, tstest_1.test)('syncRemote()', async (t) => {
    class FileBoxTest extends file_box_js_1.FileBox {
 ...
 */
(0, tstest_1.test)('toFile() with long name', async (t) => {
    ...
    const fileBox = file_box_js_1.FileBox.fromUrl(IMAGE_URL);
...
(0, tstest_1.test)('metadata', async (t) => {
    const FILE_PATH = 'tests/fixtures/data.bin';
...
(0, tstest_1.test)('fromQRCode()', async (t) => {
    ...
    const fileBox = file_box_js_1.FileBox.fromQRCode(QRCODE_VALUE);
...
(0, tstest_1.test)('toQRCode()', async (t) => {
    ...
    const fileBox = file_box_js_1.FileBox.fromBase64(QRCODE_IMAGE_BASE64, 'qrcode.png');
...
(0, tstest_1.test)('toJSON()', async (t) => {
    // const BASE64_DECODED = 'FileBoxBase64\n'
    const BASE64_ENCODED = 'RmlsZUJveEJhc2U2NAo=';
    ...
    const fileBox = file_box_js_1.FileBox.fromBase64(BASE64_ENCODED, BASE64_FILENAME);
    const jsonText = JSON.stringify(fileBox);
    t.equal(jsonText, EXPECTED_JSON_TEXT, 'should get expected json text');
    const newFileBox = file_box_js_1.FileBox.fromJSON(jsonText);
    const newBase64 = await newFileBox.toBase64();
    t.equal(newBase64, BASE64_ENCODED, 'should get base64 back');
});
(0, tstest_1.test)('toJSON() for not supported type', async (t) => {
    const BASE64_ENCODED = 'RmlsZUJveEJhc2U2NAo=';
    const buffer = Buffer.from(BASE64_ENCODED, 'base64');
    const fileBox = file_box_js_1.FileBox.fromBuffer(buffer, 'test.txt');
    t.equal(fileBox.type, file_box_type_js_1.FileBoxType.Buffer, 'should get type() as Buffer');
 ...
 */
(0, tstest_1.test)('toStream() twice for a stream', async (t) => {
    const stream = new stream_1.PassThrough();
    const box = file_box_js_1.FileBox.fromStream(stream, 'hello.dat');
...
(0, tstest_1.test)('toUuid()', async (t) => {
    const BASE64_ENCODED = 'RmlsZUJveEJhc2U2NAo=';
...
(0, tstest_1.test)('fromUuid()', async (t) => {
    const UUID = '12345678-1234-1234-1234-123456789012';
...
(0, tstest_1.test)('setUuidLoader()', async (t) => {
    class FileBoxTest1 extends file_box_js_1.FileBox {
    }
    class FileBoxTest2 extends file_box_js_1.FileBox {
    }
...
(0, tstest_1.test)('setUuidSaver()', async (t) => {
    class FileBoxTest1 extends file_box_js_1.FileBox {
    }
    class FileBoxTest2 extends file_box_js_1.FileBox {
    }
...
(0, tstest_1.test)('setUuidLoader() & setUuidSsaver() with `this`', async (t) => {
    const sandbox = tstest_1.sinon.createSandbox();
    const loader = sandbox.stub()
        .returns(await file_box_js_1.FileBox.fromQRCode('qr').toStream());
...
(0, tstest_1.test)('FileBox.validInterface()', async (t) => {
    const fileBox = file_box_js_1.FileBox.fromQRCode('test');
    /**
     * 2 OK
     */
    t.ok(file_box_js_1.FileBox.validInstance(fileBox), 'should satisfy instance validation for a FileBox instance');
    t.ok(file_box_js_1.FileBox.validInterface(fileBox), 'should satisfy interface validation for a FileBox instance');
    t.ok(file_box_js_1.FileBox.valid(fileBox), 'should satisfy interface validation');
    t.ok(fileBox instanceof file_box_js_1.FileBox, 'should be instance of FileBox');
    const copy = {};
     ...
     * 1 OK, 1 NG
     */
    t.ok(file_box_js_1.FileBox.validInstance(target), 'should pass instance validation instance test');
    t.ok(file_box_js_1.FileBox.validInterface(target), 'should pass interface validation for an object with FileBox properties');
    t.ok(file_box_js_1.FileBox.valid(target), 'should satisfy interface validation');
    t.ok(target instanceof file_box_js_1.FileBox, 'should be instance of FileBox');
    /**
     * 2 NG
     */
    delete target.size;
    t.notOk(file_box_js_1.FileBox.validInterface(target), 'should not be a valid interface if it lack any property');
    t.notOk(file_box_js_1.FileBox.valid(target), 'should not satisfy interface validation');
});
(0, tstest_1.test)('fromJSON() should keep class', async (t) => {
    const JSON_TEXT = '{"metadata":{},"name":"smoke-testing.ts","size":735,"type":7,"uuid":"82f461b9-e654-422d-aade-222d05cb02ad","boxType":7}';
    class FileBoxUuid extends file_box_js_1.FileBox {
    }
    FileBoxUuid.setUuidLoader(() => ({}));
    FileBoxUuid.setUuidSaver(() => ({}));
    const fileBoxUuid = FileBoxUuid.fromJSON(JSON_TEXT);
    const FromJsonKlass = (0, clone_class_1.instanceToClass)(fileBoxUuid, file_box_js_1.FileBox);
