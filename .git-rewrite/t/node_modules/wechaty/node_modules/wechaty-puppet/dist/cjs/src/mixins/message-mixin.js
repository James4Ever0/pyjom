...
const messageMixin = (baseMixin) => {
    class MessageMixin extends baseMixin {
        constructor(...args) {
            super(...args);
            config_js_1.log.verbose('PuppetMessageMixin', 'constructor()');
        }
        /**
        ...
        messagePayloadCache(messageId) {
            ...
            if (cachedPayload) {
                ...
                config_js_1.log.silly('PuppetMessageMixin', 'messagePayloadCache(%s) cache MISS', messageId);
            }
            return cachedPayload;
        }
        async messagePayload(messageId) {
            config_js_1.log.verbose('PuppetMessageMixin', 'messagePayload(%s)', messageId);
            if (!messageId) {
                throw new Error('no id');
            }
            /**
            ...
            */
            ...
            config_js_1.log.silly('PuppetMessageMixin', 'messagePayload(%s) cache SET', messageId);
            return payload;
        }
        messageList() {
            config_js_1.log.verbose('PuppetMessageMixin', 'messageList()');
            return [...this.cache.message.keys()];
        }
        async messageSearch(query) {
            config_js_1.log.verbose('PuppetMessageMixin', 'messageSearch(%s)', JSON.stringify(query));
            /**
            ...
            if (query?.id) {
                try {
                    ...
                    config_js_1.log.verbose('PuppetMessageMixin', 'messageSearch() payload not found for id "%s"', query.id);
                    return [];
                }
            }
            /**
            ...
            config_js_1.log.silly('PuppetMessageMixin', 'messageSearch() allMessageIdList.length=%d', allMessageIdList.length);
            ...
            config_js_1.log.silly('PuppetMessageMixin', 'messageSearch() messageIdList filtered. result length=%d', messageIdList.length);
        ...
        messageQueryFilterFactory(query) {
            config_js_1.log.verbose('PuppetMessageMixin', 'messageQueryFilterFactory(%s)', JSON.stringify(query));
        ...
        async messagePayloadDirty(id) {
            config_js_1.log.verbose('PuppetMessageMixin', 'messagePayloadDirty(%s)', id);
            await this.__dirtyPayloadAwait(payload_js_1.PayloadType.Message, id);
        }
    }
    return MessageMixin;
};
