...
const cacheMixin = (mixinBase) => {
    class CacheMixin extends mixinBase {
        ...
        dirtyPayload(type, id) {
            config_js_1.log.verbose('PuppetCacheMixin', 'dirtyPayload(%s<%s>, %s)', mod_js_1.PayloadType[type], type, id);
            /**
        ...
        onDirty({ payloadType, payloadId, }) {
            config_js_1.log.verbose('PuppetCacheMixin', 'onDirty(%s<%s>, %s)', mod_js_1.PayloadType[payloadType], payloadType, payloadId);
            const dirtyFuncMap = {
                [mod_js_1.PayloadType.Contact]: (id) => this.cache.contact.delete(id),
                [mod_js_1.PayloadType.Friendship]: (id) => this.cache.friendship.delete(id),
                [mod_js_1.PayloadType.Message]: (id) => this.cache.message.delete(id),
                [mod_js_1.PayloadType.Room]: (id) => this.cache.room.delete(id),
                [mod_js_1.PayloadType.RoomMember]: (id) => this.cache.roomMember.delete(id),
                [mod_js_1.PayloadType.Unspecified]: (id) => { throw new Error('Unspecified type with id: ' + id); },
            };
        ...
        async __dirtyPayloadAwait(type, id) {
            config_js_1.log.verbose('PuppetCacheMixin', '__dirtyPayloadAwait(%s<%s>, %s)', mod_js_1.PayloadType[type], type, id);
            ...
            try {
                ...
                config_js_1.log.warn('PuppetCacheMixin', [
                    ...
                    'error: %s',
                    'stack: %s',
                ].join('\n  '), mod_js_1.PayloadType[type], type, id, e.message, e.stack);
            }
            /**
