...
const roomMixin = (mixinBase) => {
    class RoomMixin extends mixinBase {
        constructor(...args) {
            super(...args);
            config_js_1.log.verbose('PuppetRoomMixin', 'constructor()');
        }
        async roomSearch(query) {
            config_js_1.log.verbose('PuppetRoomMixin', 'roomSearch(%s)', query ? JSON.stringify(query) : '');
            /**
            ...
            if (query?.id) {
                try {
                    ...
                    config_js_1.log.verbose('PuppetRoomMixin', 'roomSearch() payload not found for id "%s"', query.id);
            ...
            config_js_1.log.silly('PuppetRoomMixin', 'roomSearch() allRoomIdList.length=%d', allRoomIdList.length);
            ...
            config_js_1.log.silly('PuppetRoomMixin', 'roomSearch() roomIdList filtered. result length=%d', roomIdList.length);
            return roomIdList;
        }
        /**
        ...
        roomQueryFilterFactory(query) {
            config_js_1.log.verbose('PuppetRoomMixin', 'roomQueryFilterFactory(%s)', JSON.stringify(query));
        ...
        async roomPayloadDirty(id) {
            ...
            await this.__dirtyPayloadAwait(payload_js_1.PayloadType.Room, id);
        }
    }
    return RoomMixin;
};
