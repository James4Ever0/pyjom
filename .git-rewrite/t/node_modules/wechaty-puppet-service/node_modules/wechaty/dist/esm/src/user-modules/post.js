...
class PostMixin extends wechatifyMixinBase() {
    ...
    constructor(idOrPayload) {
        super();
        log.verbose('Post', 'constructor(%s)', typeof idOrPayload === 'string'
            ? idOrPayload
    ...
    async tap(type, status) {
        log.verbose('Post', 'tap(%s%s)', PUPPET.types.Tap[type], typeof status === 'undefined'
            ? ''
            : ', ' + status);
        if (!this.id) {
...
class PostImpl extends validationMixin(PostMixin)() {
}
