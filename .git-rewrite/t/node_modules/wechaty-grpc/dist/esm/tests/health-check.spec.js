...
import { grpc, google, } from '../src/mod.js';
...
test('health check smoke testing', async (t) => {
    ...
    server.addService(google.HealthService, testServer);
    await promisify(server.bindAsync
        .bind(server))(ENDPOINT, grpc.ServerCredentials.createInsecure());
    server.start();
    /**
     * Create Client
     */
    const client = new google.HealthClient(ENDPOINT, grpc.credentials.createInsecure());
    const request = new google.HealthCheckRequest();
    const response = await promisify(client.check
        .bind(client))(request);
