...
def FilesystemProcessor(info, reviewerLogs, filters={}, path_replacers={}):
    ...
    for elem in files:
        ...
        if _type == "video":
            ...
            if "audioDuration" not in info.keys(): audioInfo = None
            else:
                # audioInfo = {}
                ...
                audioInfo = {"sampleRate":sampleRate,"channels":channels,"duration":audio_duration}
            resolution = {"height":info["videoHeight"], "width":info["videoWidth"]}
            _fps = info["videoFrameRate"]
            metaInfo.update({"fps":_fps,"duration":video_duration,"resolution":resolution,"audio":audioInfo})
        elif _type == "audio":
            info = get_media_info(path)
            duration = info["duration"]
            sampleRate = info["audioSamplingRate"]
            channels = info["audioChannel"]
            metaInfo.update({"sampleRate":sampleRate,"channels":channels,"duration":duration})
            ...
            if metaInfo["suffix"].lower() == "gif":
                ...
                for e in info["streams"]:
                    ...
                    if codec_name == "gif":
                        ...
                        metaInfo.update({"duration":float(duration),"fps":eval(_fps)})
            ...
            if primarykey == "labels":
                ...
                if discard:
                    ...
                    if primarykey in filters.keys():
                        if not any([x in primary_sample_content for x in filters[primarykey]]):
                            # remove those without the label.
                            continue
                    update_subdict(fileinfo, filename,{"labels":primary_sample_content})
                ...
                if secondary_key == "yolov5":
                    ...
                    for frame in main_array_content:
                        _time, _frame, yolov5_detector = frame["time"], frame["frame"], frame["yolov5_detector"]
                        ...
                        for detected in yolov5_detector:
                            ...
                            confidence = detected["confidence"] # ignore the confidence.
                    ...
                    if secondary_key in filters.keys():
                        if not any([x in identity_dict_array.keys() for x in filters[secondary_key]]):continue# do not have the dogs.
                    ...
                    for k in new_identity_array.keys():
                        new_identity_array[k] = convoluted(new_identity_array[k],pad=1,k = 5)
                        new_identity_array[k] = [int(x>0.2) for x in new_identity_array[k]]
                        new_identity_array[k] = extract_span(new_identity_array[k],target=1) # this is span.
                        # print(new_identity_array[k])
                        # breakpoint()
                        new_identity_array[k] = [(main_time_array[a], main_time_array[b]) for a,b in new_identity_array[k]]
                    ...
                    if secondary_key in filters.keys():
                        if not any([x in new_identity_array.keys() for x in filters[secondary_key]]):continue#double check.
                    timestep = secondary_sample_content["timestep"]
                    result = {"detected_objects_timespan":new_identity_array,"timestep":timestep}
                    update_subdict(fileinfo,filename,{"yolov5":result})
                    # breakpoint()
                    # TODO: complete the convolutional span extractor.
                    # pass
                elif secondary_key == "framedifference_talib_detector": # this one is detecting the pip. active region.
                    # print("{:*^30}".format("FRAMEDIFFERECE DETECTOR"))
                    # breakpoint()
                    min_frame_threshold = 30
                    if secondary_key in filters.keys():min_frame_threshold = filters[secondary_key]
                    frameborders = []
                    ...
                    for k in main_array_content.keys():
                        ...
                        if frame_length < min_frame_threshold:continue
                        frameborders.append(frameborder)
                    update_subdict(fileinfo, filename, {"framedifference_talib_detector":frameborders})
    ...
    for k in list(fileinfo.keys()):
        ...
        mbool_condition = all([x in fileinfo[k].keys() for x in [y for y in filters.keys() if y !="meta"]])
        # print("CHECKING:",k)
        # print("CONDITION:",mbool_condition)
        # breakpoint()
        if not mbool_condition:
            fileinfo.pop(k)
    # print("FILEINFO DUMP:",fileinfo)
    # breakpoint()
    return fileinfo
