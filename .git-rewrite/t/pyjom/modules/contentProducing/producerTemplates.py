...
def getMusicCutSpans(music,music_duration,lyric_path, maxtime, mintime,mbeat_time_tolerance = 0.8):
    beats, bpm = audioOwlAnalysis(music["filepath"])
    lyric = read_lrc(lyric_path)
    lyric_times = [x["time"] for x in lyric]
    beat_duration = 60/bpm
    standard_bpm_spans = [x*beat_duration for x in range(0, math.ceil(maxtime/beat_duration)+1) if x*beat_duration >=mintime*mbeat_time_tolerance and x*beat_duration <= maxtime/mbeat_time_tolerance]

    sorted_lyrics_nearby_bpm_candidates, sorted_remained_bpm_candidates = getLyricNearbyBpmCandidates(lyric_times, beats)
    ...
    while True:
        ...
        for elem in selected_candidates:
            timespan_length = elem-timespan_length
            if inRange(timespan_length, (standard_bpm_spans[0], standard_bpm_spans[-1]),tolerance= mbeat_time_tolerance):
                # select this element.
                demanded_cut_points.append(elem)
                break
        remaining_time = music_duration-demanded_cut_points[-1]
        if remaining_time < standard_bpm_spans[0]: break
    demanded_cut_points = list(set(demanded_cut_points))
    demanded_cut_points.sort()
    for elem in demanded_cut_points.copy()[::-1]:
        if music_duration - elem < standard_bpm_spans[0]: demanded_cut_points.remove(elem)
...
def getFileCuts(filtered_info,meta_info, standard_bpm_spans, policy_names,mbeat_time_tolerance=0.8 ):
    total_cuts_dict = {}
    for file_path, cuts in filtered_info: # sample these cuts, shuffle these samples. order these samples.
            ...
            for key, content in cuts.items():
                ...
                if key == "yolov5":
                    for object_name, object_cuts in content.items():
                        file_cuts+=[x for x in object_cuts if (x[1]-x[0])>=standard_bpm_spans[0]*mbeat_time_tolerance ] # we may choose only non-overlapping cuts.
                elif key == "framedifference_talib_detector": # this is a modifier. modify all things in avaliable cuts. but it cannot work alone. is it?
        ...
        for cut in file_cuts:
            ...
            for key, content in modifiers.items():
                if key == "framedifference_talib_detector": # get the biggest span. best contain this range. no random selection.
...
def getRenderList(total_cuts,demanded_cut_spans):
    ...
    FAL_generator = infiniteShuffle(file_access_list) # infinite generator! may cause serious problems.
    ...
    for span in demanded_cut_spans:
        ...
        for filename in FAL_generator:
            ...
            for cut in file_cuts:
                ...
                if inRange(cut_duration, [span_length, span_length*1.5], tolerance=0.8):
                    selected_cut = cut
                    break
...
def petsWithMusicProducer(filtered_info, meta_info,config = {}):
    ...
    demanded_cut_spans, standard_bpm_spans = getMusicCutSpans(music,music_duration,lyric_path, maxtime, mintime)
