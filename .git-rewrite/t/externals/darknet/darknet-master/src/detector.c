...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, float thresh, float iou_thresh, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path)
{
    ...
    int iter_save_last_true = iter_save_last;
    ...
    //while(i*imgs < N*120){
    while (get_current_iteration(net) < net.max_batches + iter_save_last_true) {
        printf("LOOK AT ME: CURR_ITERATION %d\n YOUR ARTIFICIAL FINAL %d\n", get_current_iteration(net),net.max_batches+iter_save_last_true);
        if (l.random && count++ % 10 == 0) {
            ...
            if (avg_loss < 0 || get_current_iteration(net) > net.max_batches + iter_save_last_true - 100) {
                dim_w = max_dim_w;
                dim_h = max_dim_h;
            }
        ...
        */
        ...
        if (mean_average_precision > 0.0) {
            printf("\033]2;%d/%d: loss=%0.1f map=%0.2f best=%0.2f hours left=%0.1f\007", iteration-iter_save_last_true, net.max_batches, loss, mean_average_precision, best_map, avg_time);
        }
        else {
            printf("\033]2;%d/%d: loss=%0.1f hours left=%0.1f\007", iteration-iter_save_last_true, net.max_batches, loss, avg_time);
        }
        #endif
        ...
        if (calc_map && (iteration >= next_map_calc || iteration == net.max_batches + iter_save_last_true)) {
            if (l.random) {
        ...
        time_remaining = ((net.max_batches + iter_save_last_true - iteration) / ngpus)*(what_time_is_it_now() - time + load_time) / 60 / 60;
        ...
        draw_train_loss(windows_name, img, img_size, avg_loss, max_img_loss, iteration-iter_save_last_true, net.max_batches, mean_average_precision, draw_precision, "mAP%", avg_contrastive_acc / 100, dont_show, mjpeg_port, avg_time);
#endif    // OPENCV
        ...
        if (iteration >= (iter_save_last + 100) || (iteration % 100 == 0 && iteration > 1)) {
            iter_save_last = iteration; // this is not freaking right.
#ifdef GPU
            if (ngpus != 1) sync_nets(nets, ngpus, 0);
#endif
            char buff[256];
