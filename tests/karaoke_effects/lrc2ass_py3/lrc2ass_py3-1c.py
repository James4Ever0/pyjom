...
'''
using_chardet
是否启用chardet编码探测，探测结果会覆盖read_encoding
'''
using_chardet = True
...
if using_chardet:
    test = open(lrc, 'rb')
    test_buf = test.read()
    test_detect = chardet.detect(test_buf)
    encoding = test_detect['encoding']
    print('chardet: {0}, {1:.2f}%'.format(encoding, float(test_detect['confidence']) * 100))
    test.close()
...
lrc_file = open(lrc, encoding=encoding)
...
for i in range(0, len(lrc_line)):
    ...
    if offset == 0:
        ...
        if len(offset_str) != 0:
            offset_buf = int(offset_str[0].rstrip(']').lstrip('[').split(':')[1])
            if int(offset_buf) != 0:
                offset = int(offset_buf)
...
ass_line.append('; Script generated by lrc2ass_py3 1.0.0b')
...
def tag_process(tag_str: str, line: int):
    ...
    if offset != 0:
        ...
        if tag_time > 0:
            ...
            print('\n警告: 行\"{0}\"中时间标签\"{1}\"添加偏移值\"{2}ms\"后为负，该偏移不会被应用'
...
if os.path.exists(ass_fullpath):
    ...
    if len(check) == 0:
        ...
        while os.path.exists(ass_fullpath):
            file_found_count += 1
            ass_fullpath = os.path.join(os.path.dirname(), '{0}.s{1}.ass'.format(lrc_name, file_found_count)) # wrong shit!
