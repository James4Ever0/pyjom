    ...
    while True:
        try:
            ...
            def updateMyLists(
                ...
                # breakpoint()
                if len(tags)>0:
                    [int(filterTitleWithCoreTopicSet(tag, static_core_topic_list)) for tag in tags]
                else:
                    continue
                    ...
                    if len(tag_list) > 3:
                        if len(bgm_list) > 3:
                            ...
                            for cover in cover_list:
                                ...
                                # import requests
                                cropped_image = imageDogCatCoverCropAdvanced(image, dog_or_cat=dog_or_cat_original)
