...
def crop_watermark(gradx, grady, threshold=0.4, boundary_size=2,location=False):
	"""
	...
	watermark_location = ((ym,xm),(yM,xM))
	# is it true? do we need to invert the freaking mask?
	if location: return gradx[xm:xM, ym:yM, :] , grady[xm:xM, ym:yM, :], watermark_location
	else:
