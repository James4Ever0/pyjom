...
cat new_test.py | comby ':[prefix~@reloading.*$]def :[functionName](:[args]):'  'def :[functionName](:[args]):' -stdin -stdout -matcher .py |  comby 'from reloading import reloading:[^]'  'def :[functionName](:[args]):' -stdin -stdout -matcher .py -match-only
