...
def set_args():
    ...
    parser.add_argument(
        "--temperature", default=1, type=float, required=False, help="生成的temperature"
    )
    ...
    parser.add_argument(
        "--log_path",
        default="data/interact.log",
        type=str,
        required=False,
        help="interact日志存放位置",
    )
    parser.add_argument(
        "--vocab_path", default="vocab/vocab.txt", type=str, required=False, help="选择词库"
    )
    parser.add_argument(
        "--model_path", default="model/epoch40", type=str, required=False, help="对话模型路径"
    )
    parser.add_argument(
        "--save_samples_path",
        default="sample/",
    ...
    parser.add_argument(
        "--eps", default=1.0e-09, type=float, required=False, help="衰减率"
    )
    parser.add_argument(
...
def train_epoch(
    model, train_dataloader, optimizer, scheduler, logger, epoch, args, device
):
    model.train()
    # device = args.device
    # pad_id = args.pad_id
...
def getTextEncoded(textList, suffix=tokenizer.sep_token_id):
...
if __name__ == "__main__":
    ...
    train_dataloader_source = [
        (source, target) for source, target in getQQGroupChatData()
    ]
    ...
    for epoch in range(epochs):
        ...
        train_epoch(
            model,
            getTrainDataLoader(train_dataloader_source),
            optimizer,
            scheduler,
            logger,
            epoch,
        ...
        if epoch % saveEpochInterval == saveEpochInterval - 1:
