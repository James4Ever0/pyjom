...
const io_client_js_1 = require("../src/io-client.js");
...
`;
async function main() {
    const token = config_js_1.config.token;
    if (!token) {
        throw new Error('token not found: please set WECHATY_TOKEN in environment before run io-client');
    }
    console.info(welcome);
    wechaty_puppet_1.log.info('Client', 'Starting for WECHATY_TOKEN: %s', token);
    const wechaty = wechaty_builder_js_1.WechatyBuilder.build({ name: token });
    let port;
...
async function onError(e) {
    wechaty_puppet_1.log.error('Client', 'start() fail: %s', e);
    await this.quit();
    process.exit(-1);
}
main()
