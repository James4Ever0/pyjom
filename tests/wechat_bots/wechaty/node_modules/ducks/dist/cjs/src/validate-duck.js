...
exports.validateDuck = validateDuck;
/**
 *
...
function validateReducer(reducer) {
    assert_1.default.strictEqual(typeof reducer, 'function', 'default export should be function');
...
function validateActions(actions) {
    assert_1.default.ok(actions, 'should exported actions');
...
function validateOperations(operations) {
    ...
    assert_1.default.ok(operations, 'should exported operations');
...
function validateSelectors(selectors) {
    ...
    assert_1.default.ok(selectors, 'should exported selectors');
...
function validateTypes(types) {
    ...
    assert_1.default.ok(types, 'should exported types');
...
function validateSetDucks(setDucks) {
    if (setDucks) {
        assert_1.default.strictEqual(typeof setDucks, 'function', 'setDucks should be a function');
        assert_1.default.strictEqual(setDucks.length, 1, 'setDucks() should take 1 argument for the Ducks instance');
    }
}
/**
 *
...
function validateString(value) {
    assert_1.default.strictEqual(typeof value, 'string', 'should be string type: ' + value);
}
function validateActionType(actionCreator) {
    ...
    function assertFunction(creator) {
        assert_1.default.strictEqual(typeof creator, 'function', 'actionCreator is expected to be a function, but we got: ' + typeof creator + ', ' + String(creator));
    }
    function assertAsyncKey(key) {
        ...
        (0, assert_1.default)(ASYNC_KEY_LIST.includes(key), 'async action creator should with object keys: request/success/failure/cancel. we got: ' + key);
    }
}
function validateSelectorType(selector) {
    assert_1.default.strictEqual(typeof selector, 'function', 'selector should be a function');
}
function validateOperationType(operation) {
    assert_1.default.strictEqual(typeof operation, 'function', 'operation should be a function');
}
function validateMiddlewareType(middleware) {
    assert_1.default.strictEqual(typeof middleware, 'function', 'middleware should be a function');
    assert_1.default.strictEqual(middleware.length, 1, 'middleware should has 1 arguments');
    assert_1.default.match(middleware.name, /Middleware$/, 'middleware should be named as xxxMiddleware (ends with `Middleware`)');
}
function validateSagaType(saga) {
    assert_1.default.strictEqual(typeof saga, 'function', 'saga should be a function');
    assert_1.default.match(saga.name, /Saga$/, 'saga should be named as xxxSaga (ends with `Saga`)');
}
function validateEpicType(epic) {
    assert_1.default.strictEqual(typeof epic, 'function', 'epic should be a function');
    assert_1.default.match(epic.name, /Epic$/, 'epic should be named as xxxEpic (ends with `Epic`)');
}
