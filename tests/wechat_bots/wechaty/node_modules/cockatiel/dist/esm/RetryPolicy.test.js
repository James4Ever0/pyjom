...
describe('RetryPolicy', () => {
    ...
    describe('setting backoffs', () => {
        ...
        it('sets the retry attempts', async () => {
            await expect(p.delay([10, 20, 20]).attempts(1).execute(s)).to.eventually.be.rejectedWith(MyErrorA);
    ...
    it('filters error types', async () => {
        ...
        await expect(Policy.handleType(MyErrorA).retry().attempts(5).execute(s)).to.eventually.be.rejectedWith(MyErrorB);
        expect(s).to.have.been.calledTwice;
    });
    it('filters returns', async () => {
        const s = stub().onFirstCall().returns(1).onSecondCall().returns(2);
        expect(await Policy.handleWhenResult(r => typeof r === 'number' && r < 2)
            .retry()
            .attempts(5)
    ...
    it('permits specifying exponential backoffs', async () => {
        ...
        expect(await Policy.handleWhenResult(r => typeof r === 'number')
            .retry()
    ...
    it('bubbles errors when retry attempts exceeded', async () => {
        ...
        await expect(Policy.handleAll().retry().attempts(5).execute(s)).to.eventually.be.rejectedWith(MyErrorB);
        expect(s).to.have.callCount(6);
    });
