...
def delogoParser(command): return parse.parse(
    "delogo_{x:d}_{y:d}_{w:d}_{h:d}", command)
...
for command in commandString.split("|"):
    try:
        ...
        maxX, maxY = commandArguments['x'] + \
            commandArguments['w'], commandArguments['y']+commandArguments['h']
