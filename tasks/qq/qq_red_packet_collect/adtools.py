...
def makeCatOrDogConnections(group_id:str, sender_id:str, cat_or_dog:str, debug:bool=False, delete:bool=False): # whatever.
    ...
    with getNeo4jDriver().session() as session:
        ...
        if delete:
            ...
            p.MATCH.node('n1',labels='qq_group', group_id=group_id).DETACHDELETE.node('n1')
            p.MATCH.node('n2',labels ='qq_user', user_id=sender_id).DETACHDELETE.node('n2')
            p.MATCH.node('n3',labels ='ad_keyword', keyword=cat_or_dog).DETACHDELETE.node('n3')
