...
import { EMPTY, from, of, mergeMap, } from 'rxjs';
 ...
 */
const toMessage$ = (action) => from(getPuppet(action.meta.puppetId)
    ?.messagePayload(action.payload.messageId) || EMPTY).pipe(map(throwUndefined), map(extendPayloadWithPuppetId(action.meta.puppetId)));
const toContact$ = (action) => from(getPuppet(action.meta.puppetId)
    ?.contactPayload(action.payload.contactId) || EMPTY).pipe(map(throwUndefined), map(extendPayloadWithPuppetId(action.meta.puppetId)));
...
const isNotSelfMessage = (puppet) => (message) => puppet.isLoggedIn
    ...
    ? (message.talkerId || message.fromId) !== puppet.currentUserId
    : true;
const skipSelfMessage$ = (action) => of(getPuppet(action.meta.puppetId)).pipe(mergeMap(puppet => puppet
    ? from(puppet.messagePayload(action.payload.messageId)).pipe(filter(isNotSelfMessage(puppet)), mapTo(action))
    : EMPTY));
