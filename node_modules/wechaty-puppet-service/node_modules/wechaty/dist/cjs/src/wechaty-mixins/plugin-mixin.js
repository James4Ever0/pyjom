...
exports.pluginMixin = void 0;
const wechaty_puppet_1 = require("wechaty-puppet");
const plugin_js_1 = require("../plugin.js");
const pluginMixin = (mixinBase) => {
    wechaty_puppet_1.log.verbose('WechatyPluginMixin', 'pluginMixin(%s)', mixinBase.name);
    class PluginMixin extends mixinBase {
        ...
        use(...plugins) {
            ...
            wechaty_puppet_1.log.verbose('WechatyPluginMixin', 'use() total %d plugins', pluginList.length);
            const uninstallerList = [];
            for (const plugin of pluginList) {
                wechaty_puppet_1.log.verbose('WechatyPluginMixin', 'use() installing Plugin %s on Wechaty %s ...', plugin.name, this.name());
                const uninstaller = plugin(this); // <- Huan(202110): TODO: remove any
                if ((0, plugin_js_1.isWechatyPluginUninstaller)(uninstaller)) {
                    wechaty_puppet_1.log.verbose('WechatyPluginMixin', 'use() saving uninstaller for Plugin %s on Wechaty %s ...', plugin.name, this.name());
            ...
            return () => uninstallerList.forEach(uninstaller => {
                wechaty_puppet_1.log.verbose('WechatyPluginMixin', 'use() uninstalling Plugin %s on Wechaty %s ...', uninstaller.name, this.name());
                uninstaller();
                wechaty_puppet_1.log.verbose('WechatyPluginMixin', 'use() uninstalling Plugin %s on Wechaty %s ... done', uninstaller.name, this.name());
            });
        }
    }
    return PluginMixin;
};
exports.pluginMixin = pluginMixin;
