...
const is_template_string_array_js_1 = require("../pure-functions/is-template-string-array.js");
...
const MixinBase = (0, mod_js_2.wechatifyMixin)((0, mod_js_2.poolifyMixin)(mod_js_1.RoomEventEmitter)());
/**
...
class RoomMixin extends MixinBase {
    ...
    static async create(contactList, topic) {
        config_js_1.log.verbose('Room', 'create(%s, %s)', contactList.join(','), topic);
        ...
        try {
            ...
            config_js_1.log.error('Room', 'create() exception: %s', (e && e.stack) || e.message || e);
            throw e;
        }
    }
    /**
    ...
    static async findAll(query) {
        config_js_1.log.verbose('Room', 'findAll(%s)', JSON.stringify(query, stringify_filter_js_1.stringifyFilter) || '');
        try {
            ...
            this.wechaty.emitError(e);
            config_js_1.log.verbose('Room', 'findAll() rejected: %s', e.message);
            return []; // fail safe
        }
    }
    /**
    ...
    static async find(query) {
        config_js_1.log.silly('Room', 'find(%s)', JSON.stringify(query, stringify_filter_js_1.stringifyFilter));
        ...
        if (roomList.length > 1) {
            config_js_1.log.warn('Room', 'find() got more than one(%d) result', roomList.length);
        }
        for (const [idx, room] of roomList.entries()) {
            ...
            if (valid) {
                config_js_1.log.verbose('Room', 'find() room<id=%s> is valid: return it', idx, room.id);
                return room;
            }
            else {
                config_js_1.log.verbose('Room', 'find() room<id=%s> is invalid: skip it', idx, room.id);
            }
        }
        config_js_1.log.warn('Room', 'find() all %d rooms are invalid', roomList.length);
        return undefined;
    }
    ...
    constructor(id) {
        super();
        this.id = id;
        config_js_1.log.silly('Room', `constructor(${id})`);
    }
    /**
     * @ignore
     */
    ...
    async ready(forceSync = false) {
        config_js_1.log.silly('Room', 'ready()');
    ...
    async say(sayable, ...varList) {
        config_js_1.log.verbose('Room', 'say(%s, %s)', sayable, varList.join(', '));
        let msgId;
        let text;
        if ((0, is_template_string_array_js_1.isTemplateStringArray)(sayable)) {
            ...
            let mentionList = [];
            if (varList.length > 0) {
                const allIsContact = varList.every(c => contact_js_1.ContactImpl.valid(c));
                if (!allIsContact) {
                    throw new Error('mentionList must be contact when not using TemplateStringsArray function call.');
                }
                mentionList = [...varList];
                const AT_SEPARATOR = config_js_1.FOUR_PER_EM_SPACE;
            ...
            msgId = await (0, mod_js_3.deliverSayableConversationPuppet)(this.wechaty.puppet)(this.id)(sayable);
        }
        if (msgId) {
    ...
    async sayTemplateStringsArray(textList, ...varList) {
        const mentionList = varList.filter(c => contact_js_1.ContactImpl.valid(c));
        // const receiver = {
        //   contactId : (mentionList.length && mentionList[0].id) || undefined,
        //   roomId    : this.id,
        // }
        if (varList.length === 0) {
            ...
            for (; i < varListLength; i++) {
                if (contact_js_1.ContactImpl.valid(varList[i])) {
     ...
     */
    async add(contact) {
        config_js_1.log.verbose('Room', 'add(%s)', contact);
    ...
    async remove(contact) {
        config_js_1.log.verbose('Room', 'del(%s)', contact);
     ...
     */
    async del(contact) {
        config_js_1.log.warn('Room', 'del() is DEPRECATED, use remove() instead.\n%s', new Error().stack);
     ...
     */
    async quit() {
        config_js_1.log.verbose('Room', 'quit() %s', this);
    ...
    async topic(newTopic) {
        config_js_1.log.verbose('Room', 'topic(%s)', newTopic || '');
        if (!this.isReady()) {
            config_js_1.log.warn('Room', 'topic() room not ready');
        ...
        const future = this.wechaty.puppet
            ...
            .catch(e => {
            config_js_1.log.warn('Room', 'topic(newTopic=%s) exception: %s', newTopic, (e && e.message) || e);
            (0, raven_js_1.wechatyCaptureException)(e);
        });
        return future;
    }
    /**
     ...
     */
    async announce(text) {
        config_js_1.log.verbose('Room', 'announce(%s)', typeof text === 'undefined'
            ? ''
            : `"${text || ''}"`);
     ...
     */
    async qrCode() {
        config_js_1.log.verbose('Room', 'qrCode()');
    ...
    async readMark(hasRead) {
        try {
            ...
            this.wechaty.emitError(e);
            config_js_1.log.error('Room', 'readMark() exception: %s', e.message);
        }
    }
    /**
    ...
    async memberAll(query) {
        config_js_1.log.silly('Room', 'memberAll(%s)', JSON.stringify(query) || '');
        if (!query) {
    ...
    async member(queryArg) {
        config_js_1.log.verbose('Room', 'member(%s)', JSON.stringify(queryArg));
        let memberList;
        // ISSUE #622
        ...
        if (memberList.length > 1) {
            config_js_1.log.warn('Room', 'member(%s) get %d contacts, use the first one by default', JSON.stringify(queryArg), memberList.length);
        }
        return memberList[0];
    }
    /**
     ...
     */
    async memberList() {
        config_js_1.log.verbose('Room', 'memberList()');
        const memberIdList = await this.wechaty.puppet.roomMemberList(this.id);
        // if (!memberIdList) {
     ...
     */
    owner() {
        config_js_1.log.verbose('Room', 'owner()');
     ...
     */
    async avatar() {
        config_js_1.log.verbose('Room', 'avatar()');
        return this.wechaty.puppet.roomAvatar(this.id);
    }
}
class RoomImpl extends (0, mod_js_2.validationMixin)(RoomMixin)() {
}
