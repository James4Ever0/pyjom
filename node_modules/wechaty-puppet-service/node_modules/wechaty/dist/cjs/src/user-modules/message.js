...
const escape_regexp_js_1 = require("../pure-functions/escape-regexp.js");
const timestamp_to_date_js_1 = require("../pure-functions/timestamp-to-date.js");
const config_js_1 = require("../config.js");
const mod_js_1 = require("../sayable/mod.js");
...
const MixinBase = (0, wechatify_js_1.wechatifyMixin)(events_1.EventEmitter);
/**
 ...
 */
class MessageMixin extends MixinBase {
    id;
    /**
     *
     * Static Properties
     ...
     * @ignore
     */
    static Type = PUPPET.types.Message;
    /**
     * Find message in cache
     */
    static async find(query) {
        config_js_1.log.verbose('Message', 'find(%s)', JSON.stringify(query));
        ...
        if (messageList.length > 1) {
            config_js_1.log.warn('Message', 'findAll() got more than one(%d) result', messageList.length);
        }
    ...
    static async findAll(query) {
        config_js_1.log.verbose('Message', 'findAll(%s)', JSON.stringify(query) || '');
        ...
        try {
            ...
            await Promise.all(messageList.map(message => message.ready()
                .catch(e => {
                config_js_1.log.warn('Room', 'findAll() message.ready() rejection: %s', e);
            ...
            config_js_1.log.warn('Message', 'findAll() rejected: %s', e.message);
            return []; // fail safe
        }
    }
    /**
     ...
     */
    static load(id) {
        config_js_1.log.verbose('Message', 'static load(%s)', id);
        /**
    ...
    constructor(id) {
        super();
        this.id = id;
        config_js_1.log.verbose('Message', 'constructor(%s) for class %s', id || '', this.constructor.name);
    }
    /**
     * @ignore
     */
    toString() {
        ...
        if (this.type() === PUPPET.types.Message.Text
            ...
            config_js_1.log.silly('Message', 'toString() for message type: %s(%s)', PUPPET.types.Message[this.type()], this.type());
            // if (!this.#payload) {
    ...
    talker() {
        ...
        if (!talkerId) {
            ...
            config_js_1.log.warn('Message', 'talker() payload.talkerId not exist! See: https://github.com/wechaty/puppet/issues/187');
     ...
     */
    from() {
        config_js_1.log.warn('Message', 'from() is deprecated, use talker() instead. Call stack: %s', new Error().stack);
        try {
            return this.talker();
        }
        catch (e) {
    ...
    listener() {
        ...
        if (!listenerId && this.payload.toId) {
            ...
            config_js_1.log.warn('Message', 'listener() payload.listenerId should be set! See: https://github.com/wechaty/puppet/issues/187');
            console.error('Puppet: %s@%s', this.wechaty.puppet.name(), this.wechaty.puppet.version());
            console.error(new Error().stack);
        }
        if (!listenerId) {
    ...
    async toRecalled() {
        ...
        try {
            ...
            this.wechaty.emitError(e);
            config_js_1.log.verbose(`Can not retrieve the recalled message with id ${originalMessageId}.`);
        }
        return undefined;
    }
    /**
     ...
     */
    async say(sayable) {
        config_js_1.log.verbose('Message', 'say(%s)', sayable);
     ...
     */
    async recall() {
        config_js_1.log.verbose('Message', 'recall()');
    ...
    self() {
        try {
            ...
            this.wechaty.emitError(e);
            config_js_1.log.error('Message', 'self() rejection: %s', e.message);
            return false;
        }
    }
    /**
     ...
     */
    async mentionList() {
        config_js_1.log.verbose('Message', 'mentionList()');
        const room = this.room();
        ...
        if (this.payload
            ...
            const idToContact = (id) => this.wechaty.Contact.find({ id });
            const allContact = await Promise.all(this.payload.mentionIdList
                .map(idToContact));
            // remove `undefined` types because we use a `filter(Boolean)`
            return allContact.filter(Boolean);
        }
        /**
         ...
         */
        const atList = this.text().split(config_js_1.AT_SEPARATOR_REGEX);
        ...
        config_js_1.log.verbose('Message', 'mentionList() text = "%s", mentionNameList = "%s"', this.text(), JSON.stringify(mentionNameList));
        ...
        if (contactList.length === 0) {
            config_js_1.log.silly('Message', `message.mentionList() can not found member using room.member() from mentionList, mention string: ${JSON.stringify(mentionNameList)}`);
        }
        return contactList;
    }
    /**
     ...
     */
    async mention() {
        config_js_1.log.warn('Message', 'mention() DEPRECATED. use mentionList() instead. Call stack: %s', new Error().stack);
    ...
    async mentionText() {
        ...
        const textWithoutMention = mentionNameList.reduce((prev, cur) => {
            const escapedCur = (0, escape_regexp_js_1.escapeRegExp)(cur);
     ...
     */
    async ready() {
        config_js_1.log.verbose('Message', 'ready()');
        if (this.isReady()) {
            return;
        }
        ...
        if (!talkerId) {
            ...
            config_js_1.log.warn('Message', 'ready() payload.talkerId not exist! See: https://github.com/wechaty/puppet/issues/187');
            console.error('Puppet: %s@%s', this.wechaty.puppet.name(), this.wechaty.puppet.version());
            console.error(new Error().stack);
        }
        const roomId = this.payload.roomId;
        ...
        if (!listenerId && this.payload.toId) {
             ...
             */
            listenerId = this.payload.toId;
            config_js_1.log.warn('Message', 'ready() payload.listenerId should be set! See: https://github.com/wechaty/puppet/issues/187');
            console.error('Puppet: %s@%s', this.wechaty.puppet.name(), this.wechaty.puppet.version());
            console.error(new Error().stack);
        }
        if (roomId) {
     ...
     */
    async forward(to) {
        config_js_1.log.verbose('Message', 'forward(%s)', to);
        // let roomId
        // let contactId
        try {
            ...
            config_js_1.log.error('Message', 'forward(%s) exception: %s', to, e);
            throw e;
        }
    }
    /**
    ...
    date() {
        ...
        return (0, timestamp_to_date_js_1.timestampToDate)(timestamp);
    }
    /**
     ...
     */
    async toFileBox() {
        config_js_1.log.verbose('Message', 'toFileBox()');
        if (this.type() === PUPPET.types.Message.Text) {
     ...
     */
    toImage() {
        config_js_1.log.verbose('Message', 'toImage() for message id: %s', this.id);
     ...
     */
    async toContact() {
        config_js_1.log.verbose('Message', 'toContact()');
    ...
    async toUrlLink() {
        config_js_1.log.verbose('Message', 'toUrlLink()');
        if (!this.payload) {
            throw new Error('no payload');
        }
    ...
    async toMiniProgram() {
        config_js_1.log.verbose('Message', 'toMiniProgram()');
        if (!this.payload) {
            throw new Error('no payload');
        }
    ...
    async toLocation() {
        config_js_1.log.verbose('Message', 'toLocation()');
        if (!this.payload) {
            throw new Error('no payload');
        }
        ...
        return new location_js_1.LocationImpl(locationPayload);
    }
    async toPost() {
        config_js_1.log.verbose('Message', 'toPost()');
        if (!this.payload) {
            throw new Error('no payload');
        }
        ...
        const post = post_js_1.PostImpl.load(this.id);
        await post.ready();
        return post;
    }
    async toSayable() {
        config_js_1.log.verbose('Message', 'toSayable()');
        return (0, mod_js_1.messageToSayable)(this);
    }
}
class MessageImplBase extends (0, validation_js_1.validationMixin)(MessageMixin)() {
}
class MessageImpl extends (0, validation_js_1.validationMixin)(MessageImplBase)() {
}
