...
function puppetImplementation(puppet, FileBoxUuid) {
    ...
    const puppetServerImpl = {
        ...
        friendshipSearchWeixin: async (call, callback) => {
            config_js_1.log.verbose('PuppetServiceImpl', 'friendshipSearchWeixin()');
            try {
                const weixin = call.request.getWeixin();
                const contactId = await puppet.friendshipSearchWeixin(weixin);
                const response = new wechaty_grpc_1.puppet.FriendshipSearchWeixinResponse();
                if (contactId) {
                    response.setContactId(contactId);
                }
                return callback(null, response);
                ...
                return (0, grpc_error_js_1.grpcError)('friendshipSearchWeixin', e, callback);
            }
        },
