...
const MixinBase = wechatifyMixin(poolifyMixin(ContactEventEmitter)());
/**
...
class ContactMixin extends MixinBase {
    ...
    async alias(newAlias) {
        log.silly('Contact', 'alias(%s)', newAlias === undefined
            ? ''
            : newAlias);
        if (!this._payload) {
        ...
        try {
            ...
            if (newAlias && newAlias !== this._payload.alias) {
                log.warn('Contact', 'alias(%s) sync with server fail: set(%s) is not equal to get(%s)', newAlias, newAlias, this._payload.alias);
            }
        }
        catch (e) {
            this.wechaty.emitError(e);
    ...
    async ready(forceSync = false) {
        ...
        try {
            ...
            this.wechaty.emitError(e);
            log.verbose('Contact', 'ready() this.wechaty.puppet.contactPayload(%s) exception: %s', this.id, e.message);
            throw e;
        }
    }
    /**
...
class ContactImplBase extends validationMixin(ContactMixin)() {
}
class ContactImpl extends validationMixin(ContactImplBase)() {
}
export { ContactImpl, };
