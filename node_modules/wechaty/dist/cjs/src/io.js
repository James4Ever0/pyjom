...
class Io {
    ...
    wsOnOpen(ws) {
        if (this.protocol !== ws.protocol) {
            config_js_1.log.error('Io', 'initWebSocket() require protocol[%s] failed', this.protocol);
            // XXX deal with error?
        }
        config_js_1.log.verbose('Io', 'initWebSocket() connected with protocol [%s]', ws.protocol);
    ...
    wsOnMessage(data) {
        config_js_1.log.silly('Io', 'initWebSocket() ws.on(message): %s', data);
    ...
    wsOnError(e) {
        config_js_1.log.warn('Io', 'initWebSocket() error event[%s]', e && e.message);
        if (!e) {
            return;
        }
        this.options.wechaty.emitError(e);
    ...
    wsOnClose(ws, code, message) {
        if (this.state.active()) {
            config_js_1.log.warn('Io', 'initWebSocket() close event[%d: %s]', code, message);
            ws.close();
            this.reconnect();
        }
    }
