...
const contactMixin = (mixinBase) => {
    class ContactMixin extends mixinBase {
        constructor(...args) {
            super(...args);
            config_js_1.log.verbose('PuppetContactMixin', 'constructor()');
        }
        ...
        async contactSearch(query, searchIdList) {
            config_js_1.log.verbose('PuppetContactMixin', 'contactSearch(query=%s, %s)', JSON.stringify(query), searchIdList
            ...
            if (typeof query !== 'string' && query?.id) {
                try {
                    ...
                    config_js_1.log.verbose('PuppetContactMixin', 'contactSearch() payload not found for id "%s"', query.id);
            ...
            config_js_1.log.silly('PuppetContactMixin', 'contactSearch() searchIdList.length = %d', searchIdList.length);
            if (!query) {
                return searchIdList;
            }
            ...
            config_js_1.log.silly('PuppetContactMixin', 'contactSearch() searchContactPayloadList.length = %d', resultIdList.length);
            return resultIdList;
        }
        /**
        ...
        contactQueryFilterFactory(query) {
            config_js_1.log.verbose('PuppetContactMixin', 'contactQueryFilterFactory(%s)', JSON.stringify(query));
            /**
        ...
        async contactValidate(contactId) {
            config_js_1.log.silly('PuppetContactMixin', 'contactValidate(%s) base class just return `true`', contactId);
            return true;
        }
        /**
         * Issue #155 - https://github.com/wechaty/puppet/issues/155
        ...
        contactPayloadCache(contactId) {
            ...
            if (cachedPayload) {
                ...
                config_js_1.log.silly('PuppetContactMixin', 'contactPayload(%s) cache MISS', contactId);
            }
        ...
        async contactPayload(contactId) {
            ...
            config_js_1.log.silly('PuppetContactMixin', 'contactPayload(%s) cache SET', contactId);
            return payload;
        }
        async contactPayloadDirty(id) {
            config_js_1.log.verbose('PuppetContactMixin', 'contactPayloadDirty(%s)', id);
            await this.__dirtyPayloadAwait(payload_js_1.PayloadType.Contact, id);
        }
    }
    return ContactMixin;
};
exports.contactMixin = contactMixin;
