...
const cacheMixin = (mixinBase) => {
    class CacheMixin extends mixinBase {
        ...
        onDirty({ payloadType, payloadId, }) {
            ...
            const dirtyFuncMap = {
                [PayloadType.Contact]: (id) => this.cache.contact.delete(id),
                [PayloadType.Friendship]: (id) => this.cache.friendship.delete(id),
                [PayloadType.Message]: (id) => this.cache.message.delete(id),
                [PayloadType.Room]: (id) => this.cache.room.delete(id),
                [PayloadType.RoomMember]: (id) => this.cache.roomMember.delete(id),
                [PayloadType.Unspecified]: (id) => { throw new Error('Unspecified type with id: ' + id); },
            };
        ...
        async __dirtyPayloadAwait(type, id) {
            ...
            try {
                ...
                log.warn('PuppetCacheMixin', [
                    ...
                    'error: %s',
                    'stack: %s',
                ].join('\n  '), PayloadType[type], type, id, e.message, e.stack);
            }
            /**
